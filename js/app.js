...
async function search() {
    ...
    try {
        ...
        try {
            window.history.pushState({ search: query }, `搜索: ${query} - LibreTV`, `/s=${encodeURIComponent(query)}`);
        ...
        function buildCardHtml(item) {
            ...
            const typeHtml    = (item.type_name || '').toString().replace(/</g, '&lt;')
                ? `<span class="text-xs py-0.5 px-1.5 rounded bg-opacity-20 bg-blue-500 text-blue-300">${(item.type_name || '').toString().replace(/</g, '&lt;')}</span>` : '';
            const yearHtml    = item.vod_year ? `<span class="text-xs py-0.5 px-1.5 rounded bg-opacity-20 bg-purple-500 text-purple-300">${item.vod_year}</span>` : '';
            return `
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md"
                     onclick="showDetails('${safeId}','${safeName}','${sourceCode}')" ${apiUrlAttr}>
                    <div class="flex h-full">
                        ${hasCover ? `<div class="relative flex-shrink-0 search-card-img-container">
                        ...
                        <div class="p-2 flex flex-col flex-grow">
                            <div class="flex-grow">
                                ...
                                <div class="flex flex-wrap ${hasCover ? '' : 'justify-center'} gap-1 mb-2">${typeHtml}${yearHtml}</div>
        ...
        // ── 流式渲染：每个 API 返回即追加，不等所有人完成 ─────────────────
        const apiPromises = selectedAPIs.map(apiId =>
            searchByAPIAndKeyWord(apiId, query).then(results => {
                ...
                // 按名称排序后插入（保持局部有序）
                filtered.sort((a, b) => (a.vod_name || '').localeCompare(b.vod_name || '') || (a.source_name || '').localeCompare(b.source_name || ''));

                const html = filtered.map(buildCardHtml).join('');
                resultsDiv.insertAdjacentHTML('beforeend', html);
                totalCount += filtered.length;
                if (searchResultsCount) searchResultsCount.textContent = totalCount;

                // 第一批结果到达时立刻隐藏 loading
                hideLoading();
            })
        );
...
// 直接跳转 player.html（跳过 watch.html 中转，减少一次页面加载）
function playVideo(url, vod_name, sourceCode, episodeIndex = 0, vodId = '') {
    ...
    // 通过 sessionStorage 传递集数列表，避免超长 URL 且速度更快
    try {
        sessionStorage.setItem('playerEpisodes', JSON.stringify(currentEpisodes));
    } catch (e) {}
    ...
    // 直接跳 player.html，returnUrl 指向来源页
    const returnUrl = (currentPath.includes('index.html') || currentPath.endsWith('/'))
        ? currentPath : currentPath;

    const playerUrl = `player.html?id=${encodeURIComponent(vodId || '')}&source=${encodeURIComponent(sourceCode || '')}&url=${encodeURIComponent(url)}&index=${episodeIndex}&title=${encodeURIComponent(vod_name || '')}&returnUrl=${encodeURIComponent(returnUrl)}`;
