...
// ── 鉴权前缀缓存（豆瓣图片代理复用）────────────────────────────────────────
let _doubanAuthPrefix = null;
let _doubanAuthFetching = null;

async function _getDoubanAuthPrefix() {
    if (_doubanAuthPrefix !== null) return _doubanAuthPrefix;
    if (_doubanAuthFetching) return _doubanAuthFetching;
    if (window.ProxyAuth?.getAuthPrefix) {
        _doubanAuthFetching = window.ProxyAuth.getAuthPrefix().then(p => {
            _doubanAuthPrefix = p ?? '';
            _doubanAuthFetching = null;
            return _doubanAuthPrefix;
        });
        return _doubanAuthFetching;
    }
    _doubanAuthPrefix = '';
    return '';
}
...
function initDouban() {
    ...
    if (localStorage.getItem('doubanEnabled') === 'true') {
        // 提前预热鉴权缓存，使首次渲染图片时无需等待
        _getDoubanAuthPrefix();
...
async function fillAndSearchWithDouban(title) {
    ...
    if (input) {
        ...
        // ── 优化：立刻隐藏豆瓣区域、显示加载态，不等 search() 完成 ──────
        const doubanArea = document.getElementById('doubanArea');
...
function renderDoubanTags(tags) {
    ...
    currentTags.forEach(tag => {
        ...
        btn.className = `py-1.5 px-3.5 rounded text-sm font-medium transition-all duration-300 border ${
            isActive ? 'bg-pink-600 text-white shadow-md border-white' : 'bg-[#1a1a1a] text-gray-300 hover:bg-pink-700 hover:text-white border-[#333] hover:border-white'
        }`;
        btn.textContent = tag;
...
function fetchDoubanTags() {
    fetchDoubanData(`https://movie.douban.com/j/search_tags?type=movie`)
        .then(data => { movieTags = data.tags; if (doubanMovieTvCurrentSwitch === 'movie') renderDoubanTags(movieTags); })
        .catch(e => console.error("获取豆瓣热门电影标签失败：", e));
    fetchDoubanData(`https://movie.douban.com/j/search_tags?type=tv`)
        .then(data => { tvTags = data.tags; if (doubanMovieTvCurrentSwitch === 'tv') renderDoubanTags(tvTags); })
        .catch(e => console.error("获取豆瓣热门电视剧标签失败：", e));
}
...
function renderRecommend(tag, pageLimit, pageStart) {
    ...
    overlay.innerHTML = `<div class="flex items-center">
...
async function fetchDoubanData(url) {
    ...
    try {
        // ── 优化：使用缓存鉴权前缀，避免每次请求都 await ────────────────────
        let proxiedUrl;
        if (window.ProxyAuth?.getAuthPrefix) {
            proxiedUrl = PROXY_URL + encodeURIComponent(url) + (await _getDoubanAuthPrefix());
        } else if (window.ProxyAuth?.addAuthToProxyUrl) {
        ...
        try {
            ...
            if (!fallbackResponse.ok) throw new Error(`备用API请求失败`);
...
// ── 优化：并发批量获取图片代理URL，消除串行 await 循环 ────────────────────────
async function renderDoubanCards(data, container) {
    ...
    // 预取鉴权前缀（已缓存则同步返回），与图片 URL 构建一起批量处理
    const authPrefix = await _getDoubanAuthPrefix();
    ...
    for (const item of data.subjects) {
        ...
        // ── 使用缓存前缀直接拼接，无需逐张 await ───────────────────────────
        let secureProxyUrl;
        if (window.ProxyAuth?.getAuthPrefix) {
            secureProxyUrl = PROXY_URL + encodeURIComponent(item.cover) + authPrefix;
        } else if (window.ProxyAuth?.addAuthToProxyUrl) {
            // 仅在不支持 getAuthPrefix 的老版本时才单独 await（极少情况）
            secureProxyUrl = await window.ProxyAuth.addAuthToProxyUrl(PROXY_URL + encodeURIComponent(item.cover));
        } else {
            secureProxyUrl = PROXY_URL + encodeURIComponent(item.cover);
        }
...
function showTagManageModal() {
            ...
            <div class="mb-4">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-4" id="tagsGrid">
                    ${currentTags.length ? currentTags.map(tag => {
                        ...
                        return `<div class="bg-[#1a1a1a] text-gray-300 py-1.5 px-3 rounded text-sm font-medium flex justify-between items-center group">
                            <span>${tag}</span>
                            ${canDelete
    ...
    document.querySelectorAll('.delete-tag-btn').forEach(btn => {
        btn.addEventListener('click', function() { deleteTag(this.getAttribute('data-tag')); showTagManageModal(); });
    });
...
function addTag(tag) {
    ...
    const isMovie = doubanMovieTvCurrentSwitch === 'movie';
    const currentTags = isMovie ? movieTags : tvTags;
    if (currentTags.some(t => t.toLowerCase() === safeTag.toLowerCase())) { showToast('标签已存在', 'warning'); return; }
...
function deleteTag(tag) {
    ...
    if (index !== -1) {
        ...
        saveUserTags();
        if (doubanCurrentTag === tag) { doubanCurrentTag = '热门'; doubanPageStart = 0; renderRecommend(doubanCurrentTag, doubanPageSize, doubanPageStart); }
        renderDoubanTags();
